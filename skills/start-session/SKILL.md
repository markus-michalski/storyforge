...
2. **Load session context** via MCP `get_session()`
   - Returns `last_book` (a **book slug**, not a title), `last_chapter` (a chapter slug, written by `chapter-writer`/`chapter-writer-memoir`/`rolling-planner` — never a bare number), and `last_phase` (a short free-text "what's next" phrase, e.g. `"Ready for chapter-reviewer"` or a scene-recipe Goal line — not a status enum). Any of the three may still be missing (e.g. before the first chapter of a fresh session, or for books written before this wiring existed) — the DB drops empty fields from the response rather than returning them as `null`.
