...
- **Litmus test (hard gate when triggered)** — If `plot/tone.md` exists, answer EVERY question from the Litmus Test section explicitly in chat — state each question and its yes/no answer, don't fold it into a silent mental check like the other Step 8 bullets. If more than 1 answer is "no", this is a stop condition, not a checklist item: flag it to the user with specific suggested revisions and wait for their response before treating Step 8 as complete.
...
- Honor every entry in the brief's `rules_to_honor` (book CLAUDE.md ## Rules) proactively, before writing a single word — plan around `severity: block` entries during drafting rather than treating them as the PostToolUse hook's problem. The hook is a backstop for what slips through, not the primary safeguard; a block-severity violation caught post-hoc means discarding and rewriting prose that already looked finished. Honor `tone_litmus_questions` (from `plot/tone.md`) when present.
