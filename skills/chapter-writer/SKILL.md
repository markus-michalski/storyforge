...
1. **Chapter writing brief** — MCP `get_chapter_writing_brief(book_slug, chapter_slug)`. **Why:** One structured payload that bundles 14 separate context sources — `book_category`, story_anchor, recent_chapter_timelines, recent_chapter_endings, characters_present (fiction: role + knowledge + tactical profiles; memoir: relationship + person_category + consent_status + anonymization), `pov_character_inventory` (deterministic extraction of the POV character's last established physical inventory; structured `items` with source pointers; `extraction_method ∈ {frontmatter, timeline_regex, draft_heuristic, none}`; non-empty `warnings` mean **surface the gap to the user, do not invent items**; kept fresh by Step 7.8 write-back — Issue #157), `pov_character_state` (Issue #160 — four-category physical-reality snapshot: `clothing`, `injuries`, `altered_states`, `environmental_limiters`; each category extracts independently with its own `extraction_methods[cat]` and source pointers; `warnings` are outline-aware — only fired when the chapter outline references a missing category; consumed by Pre-Scene Logic Audit category 5; kept fresh by Step 7.8 write-back), `canon_brief` (Issue #161 — scoped projection of `plot/canon-log.md` (fiction) or `plot/people-log.md` (memoir); contains `current_facts` scoped to the last N review-or-later chapters, `changed_facts` with ALL `CHANGED` entries regardless of age plus `revision_impact` lists, `pov_relevant_facts` filtered by POV name, `extraction_method ∈ {section_regex, heuristic, none}`, `warnings` — non-empty means file missing or legacy format; replaces direct file read for Step 1 pre-flight, consumed by Pre-Scene Logic Audit category 3), `consent_status_warnings` (memoir only), rules_to_honor (book CLAUDE.md ## Rules with severity), callbacks_in_register, banned_phrases (book + author + global), recent_simile_count_per_chapter, tone_litmus_questions, tactical_constraints, review_handle, plus the chapter metadata. Honor every populated field while writing. Empty fields and entries in `errors` mean "not available for this chapter" — degrade gracefully (see the **Abstain from invention** rule under `## Rules > Universal`). Store the returned `review_handle` as `{review_handle}`.
...
8. **Canon log** — Use `canon_brief` from the chapter writing brief (Issue #161). **Why:** `plot/canon-log.md` grows large over a novel-length project and causes context-window truncation when read in full. `canon_brief` provides `current_facts` (last 8 review-or-later chapters) + ALL `CHANGED` entries — the facts most likely to affect the chapter being written. `warnings` non-empty = file missing or legacy format; surface to user and do not invent. If you need a fact older than the scope window, call standalone MCP `get_canon_brief(book_slug, chapter_slug, scope_chapters=N)` with a larger `N` or read the raw file.
...
9. **People log** — Use `canon_brief` from the chapter writing brief (Issue #161; memoir mode reads `plot/people-log.md`). **Why:** Same scope-window benefit as fiction's canon-log — large people-logs truncate context. `current_facts` + `changed_facts` cover what each named person *did*, *said*, *believed* in the scoped window. If `extraction_method == "none"` the file does not exist yet — create it on first chapter close (Step 7). Surface `warnings` to the user; do not invent person facts to fill gaps.
...
- **Abstain from invention.** If you cannot point to a source line in the loaded brief (including `canon_brief.current_facts`, `canon_brief.changed_facts`), `world/setting.md`, `characters/*.md` (or `people/*.md` for memoir), `plot/timeline.md`, or a previous chapter's `draft.md` for a concrete detail — items carried, character relationships, routes, location features, quoted dialogue, named objects, time anchors — do not write it. Surface the gap to the user and ask, or leave the spot empty. **Why:** every invented detail accumulates as soft canon and contradicts hard canon on the next pass. The cost of a missing detail is one question to the user; the cost of an invented detail is a continuity break that may not surface for chapters and corrupts every reader who passes through the gap before it is caught. **How to apply:** before writing any concrete claim, do a one-second source check. If the source is not immediately retrievable from the loaded data, flag it instead of guessing. The Pre-Scene Logic Audit (Step A1b / Step 2c) is the structural enforcement of this rule — emit it; do not skip it.
