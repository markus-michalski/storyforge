...
- `get_continuity_brief` had the same unbounded `canon_log_facts` load as `get_review_brief` before #500 — and worse, since it also carries every chapter's timeline grid on top. Now reuses the #500 size cap at half the budget (to leave headroom for that extra cost), reporting truncation via the same `canon_log_facts_truncated`/`canon_log_facts_total_count` fields. Ranking is inverted from `get_review_brief`: this brief has no single "current chapter" to anchor on — it's a whole-manuscript scan — so it keeps the EARLIEST chapters first under truncation instead of the most recent, since those are the foundational facts a late chapter is most likely to accidentally contradict (#501)
