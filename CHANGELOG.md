...
- `get_continuity_brief` had the same unbounded `canon_log_facts` load as `get_review_brief` before #500 — and worse, since it also carries every chapter's timeline grid on top. Now reuses the #500 size cap (originally at half the budget, tightened to a quarter by #504 once two more fields needed their own share — see the #504 entry below), reporting truncation via the same `canon_log_facts_truncated`/`canon_log_facts_total_count` fields. Ranking is inverted from `get_review_brief`: this brief has no single "current chapter" to anchor on — it's a whole-manuscript scan — so it keeps the EARLIEST chapters first under truncation instead of the most recent, since those are the foundational facts a late chapter is most likely to accidentally contradict (#501)
...
- `get_continuity_brief`'s `chapter_timelines` and `canonical_calendar` fields were still unbounded after #501 fixed `canon_log_facts` — real measurement on the Firelight reference project confirmed the risk was low today (5,132 and 2 chars respectively on a 34-chapter book) but both scale with book length, not a fixed per-book quantity, so both now get the same size-cap treatment, keeping the EARLIEST chapters/story-days first under truncation. `canon_log_facts`'s own budget was tightened from half to a quarter of `get_review_brief`'s budget so the three capped fields' combined worst case (~40,000 chars) stays safely under the ~50K real MCP output limit this repo already enforces elsewhere. New `chapter_timelines_truncated`/`_total_count` and `canonical_calendar_truncated`/`_total_count` fields mirror the existing `canon_log_facts_truncated`/`_total_count` pattern; `continuity-checker`'s `SKILL.md` gained matching truncation guards so it doesn't reconstruct duplicate `[RECONSTRUCTED]` timeline entries on top of a truncated (not actually empty) calendar (#504)
