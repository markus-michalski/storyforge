...
def parse_author_profile(path: Path) -> dict[str, Any]:
    ...
    return {
        ...
        # Language fields: written via update_author() since create_author() itself has no
        # language params, but never projected here before (found via create-author's
        # skill-eval live tier) — every skill reading state via get_author() (the mandated
        # read path per CLAUDE.md rule 1) got an empty/missing field even though profile.md
        # had the real value, e.g. the proofreader's native_language-aware explanations.
