...
12. ALWAYS use `canon_brief` from `get_chapter_writing_brief()` before writing any chapter — it carries established facts from DB (Issue #297). Contradictions break canon and reader trust. If `extraction_method == "none"`, run `scripts/migrate_canon_log_to_db.py --execute` to import from `plot/canon-log.md`.
