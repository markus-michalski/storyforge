...
12. ALWAYS use `canon_brief` from `get_chapter_writing_brief()` before writing any chapter — it carries established facts from DB (Issue #297). Contradictions break canon and reader trust. If `extraction_method == "none"`, run `scripts/migrate_canon_log_to_db.py` to import from `plot/canon-log.md`.
13. ALWAYS call `add_canon_fact(book_slug, chapter_num, subject, fact, domain=...)` after writing or revising a chapter — track new and changed facts in DB. `chapter_num` is the chapter's integer number, not the slug; pass `domain` by name (real order is `book_slug, chapter_num, subject, fact, book_num=1, domain=""`). For revisions use `is_revision=True` + `old_value` + `revision_impacts`.
...
| `import_cover_image` | Copy an externally-generated cover image (from `/storyforge:cover-artist`'s prompts, generated outside StoryForge) into `cover/art/` and record it with a draft/final flag in the `cover_images` DB table | Bridges cover-artist's prompt output to a usable file. No skill wires it yet — export-engineer still assembles its own Pandoc command manually and doesn't consult this table. Args: `book_slug`, `source_path`, optional `is_final` (default `False`) |
