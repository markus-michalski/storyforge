...
class TestValidateMemoirStructureType:
    ...
    @pytest.mark.parametrize("value", ["chronological", "thematic", "braided", "vignette"])
...
class TestSetMemoirStructureTypeValidation:
    ...
    def test_invalid_structure_type_rejected(self, server_module, content_root: Path):
        json.loads(server_module.create_book_structure(title="Memoir A", book_category="memoir"))

        result = json.loads(server_module.set_memoir_structure_type(book_slug="memoir-a", structure_type="five-act"))
    ...
    def test_rejects_fiction_book(self, server_module, content_root: Path):
        json.loads(server_module.create_book_structure(title="Fiction Plot", book_category="fiction"))
        server_module._cache.invalidate()

        result = json.loads(
            server_module.set_memoir_structure_type(book_slug="fiction-plot", structure_type="chronological")
        )
        assert "error" in result
    ...
    def test_rejects_unknown_book(self, server_module):
        result = json.loads(
            server_module.set_memoir_structure_type(book_slug="does-not-exist", structure_type="chronological")
        )
        assert "error" in result
...
class TestSetMemoirStructureTypePersistence:
    ...
    def test_writes_structure_type_to_existing_scaffold(self, server_module, content_root: Path):
        ...
        structure_path = content_root / "projects" / "year-of-glass" / "plot" / "structure.md"
        scaffold_text = structure_path.read_text(encoding="utf-8")
        assert "Memoir Structure" in scaffold_text  # #63 scaffold marker

        result = json.loads(
            server_module.set_memoir_structure_type(book_slug="year-of-glass", structure_type="chronological")
        )
    ...
    def test_overwrites_previous_choice(self, server_module, content_root: Path):
        # A user may revise the choice mid-conceptualization.
        json.loads(server_module.create_book_structure(title="Revision Memoir", book_category="memoir"))

        json.loads(server_module.set_memoir_structure_type(book_slug="revision-memoir", structure_type="chronological"))
        json.loads(server_module.set_memoir_structure_type(book_slug="revision-memoir", structure_type="braided"))

        structure_path = content_root / "projects" / "revision-memoir" / "plot" / "structure.md"
        meta, _ = parse_frontmatter(structure_path.read_text(encoding="utf-8"))
        assert meta["structure_type"] == "braided"

    def test_creates_file_when_legacy_memoir_lacks_structure_md(self, server_module, content_root: Path):
        # Memoir books scaffolded before #63 do not have plot/structure.md.
        # The tool must create it rather than failing.
        json.loads(server_module.create_book_structure(title="Legacy Memoir", book_category="memoir"))

        # Simulate a legacy memoir by deleting the scaffolded structure.md.
        structure_path = content_root / "projects" / "legacy-memoir" / "plot" / "structure.md"
        structure_path.unlink()
        assert not structure_path.exists()

        result = json.loads(
            server_module.set_memoir_structure_type(book_slug="legacy-memoir", structure_type="thematic")
        )
        assert result.get("success") is True
    ...
    def test_all_four_types_persist(self, server_module, content_root: Path, structure_type: str):
        # Each of the four valid types persists identically.
        title = f"Memoir {structure_type}"
        json.loads(server_module.create_book_structure(title=title, book_category="memoir"))

        slug = title.lower().replace(" ", "-")
        result = json.loads(server_module.set_memoir_structure_type(book_slug=slug, structure_type=structure_type))
        assert result.get("success") is True
